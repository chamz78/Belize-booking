    ...
    public function addToCart(Request $request)
    {
            ...
            if (!empty($this->extra_price)) {
                foreach ($this->extra_price as $k => $type) {
                    if (isset($extra_price_input[$k]) and $extra_price_input[$k]['enable'] and $extra_price_input[$k]['enable'] != 'false') {
                        ...
                        switch ($type['type']) {
                            case "one_time":
                                $type_total = $type['price'] * $total_room_selected;
                                break;
                            case "per_day":
                                $type_total = $type['price'] * ceil($duration_in_hour / 24) * $total_room_selected;
                                break;
                        }
        ...
        if($this->isDepositEnable())
        {
            switch ($this->getDepositType()){
                case "percent":
                    $booking->deposit = $booking->total * $this->getDepositAmount() / 100;
                    break;
                default:
        ...
        if ($this->enable_extra_price) {
            ...
            if (!empty($booking_data['extra_price'])) {
                foreach ($booking_data['extra_price'] as $k => &$type) {
                    ...
                    switch ($type['type']) {
                        case "one_time":
                            $type['price_type'] .= "/".__("room");
                            break;
                        case "per_day":
                            $type['price_type'] .= '/' . __('day')."/".__("room");
                            break;
                    }
